...
const PickedCountries: FC<IPickedCountries> = ({ countries,removeSelectedCountry,fullWidth }) => {
  ...
  return (
    <div className={` ${fullWidth ? 'w-full mt-5 flex' : 'w-1/2'}`}>
      {!fullWidth && countries.length ? <p>Click on the flag if you want to remove selected county.</p> : null}
      ...
      {countries.map((country: CountryInfo) => {
        return (
          <img
            ...
            className={`m-1 w-40 cursor-pointer  ${!fullWidth && "hover:opacity-50"}`}
